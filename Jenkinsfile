    ...
    environment{
        ...
        SonarQube_Project_Key='sqs:k-tanishakapoor-develop'
         SonarQube_Project_Name='sqs:project-tanishakapoor-develop'
          ...
          scannerHome = tool name :'sonar-scanner-dotnet',type:'hudson.plugins.sonar.MSBuildSQRunnerInstallation'
    }
    stages{
        stage('nuget store'){
            steps{
        ...
        stage('start sonarqube analysis'){
            steps{
                withSonarQubeEnv('Test_Sonar'){
                    bat "dotnet \"${scannerHome}/SonarScanner.MsBuild.dll\" begin /k:${env.SonarQube_Project_Key} /n:${env.SonarQube_Project_Name}
